...
if(isset($_GET['edit_id'])){
    ...
    if(!empty($text)){
        ...
        if(empty($error)){
            $result = $Controller->shugo_data_duplication_check($text);
            if(empty($result)){
                $Controller->edit_shugo($id,$text);
                header('location: myWord.php?edit=1');
            }else{
...
if(isset($_GET['delete_id'])){
    $id = $_GET['delete_id'];
    $Controller->shugo_delete($id);
    header('location: myWord.php?delete=1');
}
$result = $Controller->select_shugodata_where_id($id);
?>
<!DOCTYPE html>
<html lang="ja">
<head>
...
<body>
    ...
    <div class="container css-bg">
        ...
        <form method="post" action="edit_delete_shugo.php?edit_id=<?= $result['id'] ?>">
        ...
        $(function(){
            ...
            $("#delete").click(function(){
                if(!confirm('削除しますか？')){
                    ...
                    window.location.href = 'edit_delete_shugo.php?delete_id=<?=$id ?>'
                }
            })
        })
    </script>
